...
C******************************************************************************
C
C   Convolution of Green functions with moment tensor.
C   Program modified from CONVOL (of O. Coutant)
...
c pro posun vlevo dej vhodne dt0<0 v sekundach
c ...nezapomen, ze trojuhelnikova funkce ics=4 je centrovana v 0 case
c ...takze pokud chceme aby zacinala v nule dame kladny posun (t0/2)
      dt0=0.
...
C Set the different source-time functions 
cccccccccccccccccccccccccc
c  moment time fctn fsource (OK)
c      ics=7               ! real step in displacement
c      icc=2               ! output time series will be velocity
cccccccccccccccccccccccccc
c  moment time fctn fsource (OK)
c      ics=2                ! Bouchon's smooth step in displacement
c      t0=1.2               ! smoothing step in time (in sec)
c      icc=2                ! output time series will be velocity
cccccccccccccccccccccccccc
c  moment time fctn fsource (OK)
c      ics=4                ! triangle in displacement
c      t0=5.0               ! triangle base in seconds (triangle is centred on zero time)
...
cccccccccccccccccccccccccc
c  moment time TIME-DELAYED DIRAC in velocity (OK)
c      ics=20                ! time TIME-DELAYED DIRAC in velocity
c      t0=32.                ! time delay in seconds of the dirac
c      icc=1                 ! POZOR da to veloc (protoze zdrojfce je derivace momentu)
cccccccccccccccccccccccccc
c  moment time TIME-DELAYED Brune's pulse with duration t1 and timeshifted by t0
c      ics=21                ! 
c      t0=10.
c      t1=0.
c      icc=1                ! POZOR formalne displ ale da to veloc (protoze zdrojfce je derivace momentu)
cccccccccccccccccccccccccc
      icc=icc-1
c
...
c      read(14,*)
c      read(14,*)
C cykl pres prijimace, jen cteni polohy
      do ir=1,nr
...
c zde je nasobeni source-time fce., jeho derivace a time-shift
...
c jen ladeni source-time fce. pokud ji chci vyplotit
c           ux(jf,ir)=us
c           uy(jf,ir)=us
c           uz(jf,ir)=us
...
C pomocny soubor se zdrojovou funkci (pro ladeni)
c        OPEN (19,form='formatted',file='test1.dat') !pro ladeni
c        REWIND(19)
c        DO if=1,nt
c          WRITE(19,*) REAL(ux(if,ir)),IMAG(ux(if,ir)) !pro ladeni
c        ENDDO
...
C pomocny soubor se zdrojovou funkci (pro ladeni)
c        OPEN (29,form='formatted',file='test2.dat') !pro ladeni
c        REWIND(29)
c        DO if=1,nt
c          WRITE(29,*) REAL(ux(if,ir)),IMAG(ux(if,ir)) !pro ladeni
c        ENDDO
...
c TYPE=20               Source = Dirac en deplacement, shifted in time by t0
      if (type.eq.20) then
        fsource=exp(-ai*omega*t0)
      endif

c TYPE=21               Source = Brune's pulse of width t1, shifted in time by t0
      if (type.eq.21) then
        fsource=exp(-ai*omega*t0)/(ai*omega*t1/pi2+1.)**2
      endif
